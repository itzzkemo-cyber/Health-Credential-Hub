...
export const usersTable = pgTable(
  "users",
  {
    ...
    phone: text("phone"),
    // Set only by a server-side verification flow. A phone number without this
    // timestamp must never be treated as proof that the employee controls it.
  ...
  (table) => [
    ...
    check(
      "users_phone_verification_requires_phone",
      sql`${table.phoneVerifiedAt} is null or ${table.phone} is not null`,
    ),
  ],
);
