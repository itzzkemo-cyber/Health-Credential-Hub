...
export default function Settings() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">{t('common.settings')}</h1>
        <p className="mt-1 text-sm text-muted-foreground sm:text-base">{t("settings_page.subtitle")}</p>
      </div>
      ...
      <div className="grid gap-6">
        ...
        <Card className="hover-elevate">
          <CardHeader>
            ...
            <CardDescription>{t("settings_page.language_region_desc")}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="space-y-0.5">
                <Label>{t("settings_page.interface_language")}</Label>
                <p className="text-sm text-muted-foreground">{t("settings_page.interface_language_desc")}</p>
              </div>
              <Select value={language} onValueChange={(v) => setLanguage(v as 'ar' | 'en')}>
                <SelectTrigger className="min-h-11 w-full sm:w-[180px]">
                  <SelectValue placeholder={t("settings_page.interface_language")} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">{t("settings_page.calendar_desc")}</p>
              </div>
              <Switch />
            </div>
          </CardContent>
          ...
          <CardContent className="space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">{t("settings_page.theme_desc")}</p>
              </div>
              <Select value={theme} onValueChange={(v) => setTheme(v as any)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="system">{t("settings_page.system")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </CardContent>
          ...
          <CardContent className="space-y-6">
            <div className="flex items-center justify-between gap-4">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">{t("settings_page.expiry_warnings_desc")}</p>
              </div>
        ...
        <div className="mt-4 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <Button variant="outline" className="min-h-11">{t("settings_page.reset")}</Button>
          <Button onClick={handleSave} className="min-h-11">{t('common.save')}</Button>
        </div>
