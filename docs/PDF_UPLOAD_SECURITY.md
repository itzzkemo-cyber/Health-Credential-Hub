...
- Encrypted PDFs, forms (including AcroForm/XFA), digital signatures, embedded files, JavaScript or non-URI actions, RichMedia, and unsupported JPEG2000/JBIG2 image streams are **rejected**, not silently kept or stripped. A narrowly validated inline `http:`, `https:`, or `mailto:` hyperlink annotation may pass preflight because the image-only rebuild discards the annotation and its action entirely. Indirect actions, destinations, extra action keys, and every active action remain fail-closed. The employee should request a flat, unsigned copy from the issuer where appropriate; this service does not attest to an original digital signature.
...
- Selecting a PDF sends an authenticated **ephemeral review request** with `X-HealthDocs-PDF-Text: review`. Extraction and the image-only rebuild run inside the bounded sanitizer worker, but neither the original nor rebuilt bytes are written to object storage and the temporary grant is removed before the response. The source text is not stored, logged, added to audit events, or returned to the browser. The API returns only bounded credential-field suggestions with `Cache-Control: private, no-store`; the employee must review and explicitly apply them. A scanned/image-only PDF may yield no suggestions and must be entered manually. Choosing **Save Document** later performs a fresh normal sanitizer pass and only then writes the rebuilt PDF to private storage.
- External PDF OCR remains deliberately **disabled**. The existing external OCR API continues to accept only image MIME types, so this path adds no external processor or new data transfer. Enabling external PDF OCR later requires a documented processor, region, retention, failure, and operator-setup review.
...
- Abandoned-upload cleanup is owner-scoped and idempotent. A valid cleanup replay with no actor-owned grant returns `204` without touching storage or writing an audit event; that status means only that the caller has no remaining cleanup capability, not that the storage provider has no orphan. Provider lifecycle and reconciliation remain an operator requirement.
...
The parent enforces a **20-second wall-clock deadline**, kills a timed-out worker, waits for its exit, caps rebuilt output at 8 MiB, caps opt-in extracted text at 24 KiB, and discards bounded parser diagnostics. The child has a **192 MiB V8 heap limit**, constrained file-read permission for its code/dependencies, no Node filesystem-write permission, and no permission to spawn children or workers. Native canvas loading requires the explicit addon permission. PDF parsing has additional object-count/depth limits and canvas allocation checks. Normal save uploads do not invoke text extraction; it runs only for the explicit ephemeral review header.
...
Synthetic automated coverage includes image-only multi-page rebuild, removal of source metadata/text, strict discard of safe inline URI links, rejection of indirect or active actions, compressed JavaScript, active dictionaries, attachments, encryption declaration, page/pixel/size limits, missing/stalled/over-output workers, environment minimization, readiness, explicit local-text opt-in, bounded review suggestions, PDF grant finalization, rejection before storage, and frontend PDF-kind propagation.
