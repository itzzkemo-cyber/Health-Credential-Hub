...
| Administration | HTTP route tests plus an opt-in real PostgreSQL + HTTP + private filesystem drill cover password/TOTP login, admin delegation, replay denial, scope, file denial, session revocation and persisted audit. The immutable owner account selected by `PROTECTED_MFA_USER_ID` uses TOTP and cannot disable it; all other administrators use password re-authentication for sensitive operations. | Limiting TOTP to one owner account is an explicit owner risk acceptance and is weaker than MFA for every privileged account. Delegation/revocation and cross-facility file denial remain synthetic-drill evidence, not live acceptance. |
...
| External processors | Invitations, employee activation email OTP, recovery and alerts use Resend; OCR is disabled and n8n remains opt-in. The retired Twilio flow is not used at runtime. | Verify the Resend contract, sender domain, data retention, tracking, region/subprocessors, transfer safeguards and deletion terms. No OCR/n8n enablement is authorized by this review. |
...
4. **Providers and transfers:** review processor contracts/DPA, subprocessors,
   ...
   Render, Supabase, Resend and any later OCR or workflow
   recipient.
5. **Security operations:** document the single protected MFA account and the
   accepted risk for privileged accounts without TOTP; maintain a named backup/key
